#!/usr/bin/env python3
"""
YouTube Video MCP Server with Secure Admin Flow & Cookie Management
"""
...
def verify_cookies_content(content: str) -> tuple[bool, str]:
    if not is_netscape_format(content):
        return False, "Invalid cookie format. Netscape tab-separated cookies require at least 7 columns per line."
    ...
    # Using iOS client avoids 'The page needs to be reloaded' prompt completely
    opts = {
        "quiet": True,
        "no_warnings": True,
        "skip_download": True,
    ...
    try:
        ...
        if "Sign in to confirm you’re not a bot" in err_msg:
            return False, "YouTube rejected these cookies: Bot confirmation failed. Please export fresh cookies."
...
# ---------------------------------------------------------------------------
# ASGI App Construction (Fixing 404 / 502 on /mcp)
# ---------------------------------------------------------------------------

class CombinedApp:
    """Dispatches /mcp requests directly to the MCP sub-app, and all other routes to Starlette."""
    def __init__(self, mcp_app: ASGIApp, admin_app: ASGIApp):
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send):
        if scope["type"] == "http":
            path = scope.get("path", "")
            # Route both /mcp and /mcp/ directly to the MCP transport handler
...
def create_app() -> CombinedApp:
    ...
    admin_routes = [
        Route("/", endpoint=lambda r: RedirectResponse(url="/admin")),
    ...
    admin_app = Starlette(debug=False, routes=admin_routes)
    return CombinedApp(mcp_app=mcp_app, admin_app=admin_app)
